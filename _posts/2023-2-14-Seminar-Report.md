...
Now that we know about E-P and P-E, lets analyze the performance of the four LLMs when explanations are given. The table below summarizes the results for all the three datasets: 
...
We see that the performance has very small to moderate change on three models except for text-danvinci-002 which achieved better performance on E-P than few shot prompting. On InstructGPT, both QA datasets improve the performance on E-P and for NLI task, E-SNLI improves the performance on P-E.
